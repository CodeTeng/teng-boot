...
function onDragMove(e: MouseEvent | TouchEvent) {
  ...
  const btnSize = 36
  const maxSliderLeft = trackRect.width - btnSize
  if (maxSliderLeft <= 0) return
  const deltaX = clientX - startX
  let newLeft = startLeft + deltaX
  newLeft = Math.max(0, Math.min(maxSliderLeft, newLeft))
  sliderLeft.value = newLeft

  const imgEl = imageRef.value?.querySelector('img')
  if (imgEl) {
    const imgW = imgEl.clientWidth || 310
    const maxPieceOffset = imgW - PIECE_SIZE
    const percent = maxSliderLeft > 0 ? newLeft / maxSliderLeft : 0
    pieceOffset.value = percent * maxPieceOffset
...
async function onDragEnd() {
  ...
  if (!track) return
  const trackRect = track.getBoundingClientRect()
  const maxSliderLeft = trackRect.width - 36
  const minDragPercent = 0.05
  if (maxSliderLeft > 0 && sliderLeft.value / maxSliderLeft < minDragPercent) {
    sliderLeft.value = 0
    pieceOffset.value = 0
  ...
  const imgEl = imageRef.value?.querySelector('img')
  if (!imgEl || !captchaData.value) return
  const imgW = imgEl.clientWidth || 310
  const maxPieceOffset = imgW - PIECE_SIZE
...
.slider-captcha {
  ...
  &__track-fill {
    ...
    top: 0;
    height: 100%;
    background: linear-gradient(90deg, #f5c76e, #d97706);
    pointer-events: none;
    border-radius: 0;

    &--success {
      background: linear-gradient(90deg, #7bc99a, #18a058);
    }
  }
  ...
  &__track-btn {
    ...
    &--fail {
      ...
      background: #d03050;
      color: #fff;
      box-shadow: 0 2px 8px rgba(208, 48, 80, 0.35);
    }
  }
